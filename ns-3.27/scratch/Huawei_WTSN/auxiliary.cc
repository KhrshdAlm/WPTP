...
#define HUAWEI_WTSN_DENKR_ESSENTIALS_AUXILIARY_C_OVERRIDE
#define NO_HUAWEI_WTSN_DENKR_ESSENTIALS_AUXILIARY_C_OVERRIDE_FUNCTIONS
...
//---------------------------------------------------------------------------------
	...
	#endif
#endif
#include "DenKr_essentials/function_creator.h"
extern "C" {
	#include "DenKr_essentials/getRealTime.h"
...
#define STORAGE_FILES_PATH_ABSOLUTE_MANUAL_FUNCTION_ROOTPATH "1 SimResults/"
	char* find;
	if( (find=strstr(argv,"Huawei_WTSN/ns") ) == NULL){
		printfc(yellow,"WARNING: ");printf("Couldn't resolve designated Storage-File Subpath. Thus using the specified MANUAL_ABSOLUT_PATH.\n");
		...
		for(;*find!='/';find++){}
		int argv_sublen=find-argv+1;//First Character, up to (and including) the trailing '/', excluding a '\0'
		StorageFilesPath_length = argv_sublen + sizeof(STORAGE_FILES_PATH_ABSOLUTE_MANUAL_FUNCTION_ROOTPATH);
		StorageFilesPath = (char*)malloc(StorageFilesPath_length);
		snprintf(StorageFilesPath,argv_sublen+1,"%s",argv);
		snprintf(StorageFilesPath+argv_sublen,StorageFilesPath_length-argv_sublen,"%s",STORAGE_FILES_PATH_ABSOLUTE_MANUAL_FUNCTION_ROOTPATH);
		return 0;
	}
}
...
#undef NO_HUAWEI_WTSN_DENKR_ESSENTIALS_AUXILIARY_C_OVERRIDE_FUNCTIONS
#undef HUAWEI_WTSN_DENKR_ESSENTIALS_AUXILIARY_C_OVERRIDE
